...
var calculateChecksum = require('./helpers').calculateChecksum;
...
var spheroResponseTemplate = {
  ...
  LEN:  4,
  DATA: 5,
  CHK:  5 //The checksum offset must be adjusted if LEN > 1
};
...
var spheroMinimumPacketSize = 6; //Smallest packet size is 5 bytes (simple acknowledgment packet)
...
exports.spheroResponseParser = function spheroResponseParser() {
  ...
  console.log("[parser] [parseBuffer] " + JSON.stringify(packet));
        if (packet) {
  ...
  console.log("[parser] [parseBuffer] " + error);
...
var parseAcknowledgment = exports._parseAcknowledgment = function(aBuffer) {
  ...
  /*
    Minimum response packet length is 6
     SOP|SOP2|MRSP|SEQ|LEN|<DATA>|CHK
   */
  if (aBuffer.length >= spheroMinimumPacketSize) { //Don't bother until we get a minimum sized packet
    ...
    responsePacket.LEN = aBuffer.readUInt8(spheroResponseTemplate.LEN); //Includes CHK byte
    responsePacket.DATA = new Buffer(responsePacket.LEN - 1);
    //We have a minimum packet length, but if we can't get the CHK yet in case this packet is bigger than that
    expectedPacketLength = responsePacket.LEN + spheroHeaderLength; //LEN is the size of <DATA> + CHK so we need to include the header bytes
    if (aBuffer.length >= expectedPacketLength) { //Make sure there is enough data in the buffer to complete the packet
      ...
      responsePacket.CHK = aBuffer.readUInt8(spheroResponseTemplate.DATA + responsePacket.LEN - 1);
      calculatedChecksum = calculateChecksum(aBuffer.slice(spheroResponseTemplate.MRSP, spheroResponseTemplate.CHK + responsePacket.LEN - 1));
...
var parseAsynchronousMessage = exports._parseAsynchronousMessage = function(aBuffer) {
  ...
  /*
   ...
   SOP1|SOP2|ID_CODE|SEQ|LEN|<DATA>|CHK
   */
  if (aBuffer.length >= spheroMinimumPacketSize) { //Don't bother until we get a minimum sized packet
    ...
    responsePacket.SEQ = aBuffer.readUInt8(spheroAsynchronousTemplate.SEQ);
    responsePacket.LEN = aBuffer.readUInt8(spheroAsynchronousTemplate.LEN); //Includes CHK byte
    responsePacket.DATA = new Buffer(responsePacket.LEN - 1);
    //We have a minimum packet length, but if we can't get the CHK yet in case this packet is bigger than that
    expectedPacketLength = responsePacket.LEN + spheroHeaderLength; //LEN is the size of <DATA> + CHK so we need to include the header bytes
    if (aBuffer.length >= expectedPacketLength) { //Make sure there is enough data in the buffer to complete the packet
      //Ok, now we are sure we have the entire packet we can grab the CHK data
      responsePacket.CHK = aBuffer.readUInt8(spheroAsynchronousTemplate.DATA + responsePacket.LEN - 1);
      calculatedChecksum = calculateChecksum(aBuffer.slice(spheroAsynchronousTemplate.ID_CODE, spheroAsynchronousTemplate.CHK + responsePacket.LEN - 1));
      if (calculatedChecksum === responsePacket.CHK) {
        //Good packet
...
var scanForPacket = exports._scanForPacket = function(aBuffer) {
  console.log("[parser] [scanForPacket] received data " + JSON.stringify(aBuffer) + " ("+aBuffer.length+")");
