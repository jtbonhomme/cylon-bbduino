...
/**
 * Speeds up Bbduino motors
 *
 * @return {null}
 * @publish
 */
Adaptor.prototype.speed = function(lSpeed, rSpeed) {
  Cylon.Logger.info("Set Bbduino's motors speed : " + lSpeed + " ; " + rSpeed);
  this.bbduino.write(commands.api.speed(lSpeed, rSpeed), function(err){
    if(err) {
	  this.emit("err", err);
	}
	else {
 ...
 * @publish
 */
Adaptor.prototype.servo = function(pos, callback) {
  Cylon.Logger.info("Set Bbduino's servo position : " + pos);
  this.bbduino.write(commands.api.servo(pos), function(err){
    if(err) {
	  this.emit("err", err);
	}
	else {
